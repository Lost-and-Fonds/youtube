...
spl_autoload_register(static function (string $class): void {
    foreach (['YouTube\\' => __DIR__ . '/../src/', 'Stashd\\PluginSdk\\' => __DIR__ . '/../../../plugin-sdk/src/'] as $prefix => $root) {
...
function ytAssert(bool $ok, string $message): void { if (! $ok) throw new RuntimeException($message); }
...
final class YtHttp implements HttpClient
{
    public function request(string $method, string $url, array $headers = [], ?string $body = null, ?string $credential = null): HttpResponse
    {
        ...
        if (str_contains($url, 'oembed')) return new HttpResponse(200, inlineBody: '{"title":"Video","thumbnail_url":"https://i.ytimg.com/x.jpg"}');
        if (str_contains($url, 'channel/') || str_contains($url, '@fixture')) return new HttpResponse(200, inlineBody: '<meta property="og:title" content="Channel"><script>"channelId":"UCfixture123"</script>');
...
final class YtStage implements StagingArea
{
    ...
    public function write(string $relativePath, string $content, ?string $mediaType = null): StagedArtifact { return new StagedArtifact($relativePath, $mediaType ?? 'application/octet-stream', strlen($content)); }
    public function stage(string $relativePath, ?string $mediaType = null): StagedArtifact { $this->paths[] = $relativePath; return new StagedArtifact($relativePath, $mediaType ?? 'application/octet-stream', 10); }
}

final class YtHelper implements HelperRunner
{
    /** @var list<string> */ public array $args = [];
    public function run(string $name, array $arguments = []): HelperResult { ytAssert($name === 'yt-dlp', 'wrong helper'); $this->args = $arguments; return new HelperResult(0, "/staging/youtube-vid1.mp4\n/staging/youtube-vid1.info.json\n/staging/youtube-vid1.jpg\n"); }
}
...
echo "YouTube provider contract: PASS\n";
